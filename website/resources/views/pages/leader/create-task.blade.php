        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="p-6">
                <form action="{{ route('leader.projects.store-task') }}" method="POST" id="createTaskForm">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <input type="text" name="card_title" id="card_title" required
                                ...
                                placeholder="Enter task title" value="{{ old('card_title') }}">
                            @error('card_title')
                        ...
                        <div>
                            ...
                            @error('user_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-sm text-gray-500">Users with (work) status are currently working on a task
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="priority" id="priority" required
                                ...
                                <option value="medium"
                                    {{ old('priority') == 'medium' || old('priority') == '' ? 'selected' : '' }}>
                                    🟡 Medium Priority
                                </option>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required
                                ...
                                <option value="todo"
                                    {{ request('status') == 'todo' || old('status') == 'todo' || (!request('status') && !old('status')) ? 'selected' : '' }}>
                                    📋 To Do
                                </option>
                                <option value="in_progress"
                                    {{ request('status') == 'in_progress' || old('status') == 'in_progress' ? 'selected' : '' }}>
                                    🔄 In Progress
                                </option>
                                <option value="review"
                                    {{ request('status') == 'review' || old('status') == 'review' ? 'selected' : '' }}>
                                    👀 Review
                                </option>
                                <option value="done"
                                    {{ request('status') == 'done' || old('status') == 'done' ? 'selected' : '' }}>
                                    ✅ Done
                                </option>
                            </select>
                            @error('status')
                        ...
                        <div>
                            ...
                            <input type="date" name="due_date" id="due_date" min="{{ date('Y-m-d') }}"
                                ...
                                value="{{ old('due_date') }}">
                            <p class="mt-1 text-sm text-gray-500">Must be between today and project deadline
                        ...
                        <div>
                            ...
                            <input type="number" name="estimated_hours" id="estimated_hours" step="0.5"
                                min="0" max="999.99"
                                ...
                                placeholder="e.g., 8.5" value="{{ old('estimated_hours') }}">
                    ...
                    <!-- Form Actions -->
                    <div
                        class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 mt-8 pt-6 border-t border-gray-200">
                        ...
                        <div
                            class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 sm:flex-1 sm:justify-end">
                            <button type="submit" name="action" value="create_and_continue"
                            ...
                            <button type="submit" name="action" value="create_and_view"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            dueDateInput.addEventListener('change', function() {
                ...
                if (selectedDate) {
                    ...
                    const maxDate = new Date(projectDeadline);
                    ...
                    if (selected < minDate) {
                        ...
                        alert(
                            'Due date cannot be after project deadline ({{ \Carbon\Carbon::parse($project->deadline)->format('M d, Y') }}).');
                        this.value = '';
                    }
                }
            });
