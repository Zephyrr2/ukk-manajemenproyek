    ...
    <div class="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            @elseif($project->status === 'submitted')
                <span class="bg-yellow-100 text-yellow-800 px-4 py-2 rounded-lg font-medium flex items-center justify-center">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @elseif($project->status === 'done')
                <span class="bg-green-100 text-green-800 px-4 py-2 rounded-lg font-medium flex items-center justify-center">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
            ...
            @elseif($project->status === 'rejected')
                <span class="bg-red-100 text-red-800 px-4 py-2 rounded-lg font-medium flex items-center justify-center">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        @if($project->submission_note)
                            <div>
                                <label class="text-sm font-medium text-gray-500">Submission Note</label>
                                <p class="mt-1 text-sm text-gray-700">{{ $project->submission_note }}</p>
                            </div>
                        @endif

                        @if($project->review_note)
                            <div>
                                <label class="text-sm font-medium text-gray-500">Review Note</label>
                                <p class="mt-1 text-sm text-gray-700">{{ $project->review_note }}</p>
                                @if($project->reviewer)
                                    <p class="mt-1 text-xs text-gray-500">By {{ $project->reviewer->name }}</p>
                                @endif
                            </div>
                        @endif
                        ...
                        @if($project->submitted_at)
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $project->submitted_at->format('M d, Y H:i') }}</p>
                            </div>
                        @endif

                        @if($project->reviewed_at)
                            <div>
                                <label class="text-sm font-medium text-gray-500">Reviewed At</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $project->reviewed_at->format('M d, Y H:i') }}</p>
                            </div>
                        @endif
                    </div>
                </div>
...
<div id="submitModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
        <div class="mt-3">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-medium text-gray-900">Submit Project for Review</h3>
                <button onclick="closeSubmitModal()" class="text-gray-400 hover:text-gray-500">
            ...
            <form id="submitProjectForm" onsubmit="submitProject(event)">
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        Submission Note <span class="text-gray-400">(Optional)</span>
                    </label>
                    <textarea id="submission_note" name="submission_note" rows="4"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="Add any notes or comments for the admin..."></textarea>
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-md p-3 mb-4">
                    <div class="flex">
                        ...
                        <p class="text-sm text-yellow-700">Once submitted, the project will be sent to admin for review.</p>
                    </div>
                </div>
...
async function submitProject(event) {
    ...
    try {
        ...
        const response = await fetch('{{ route("leader.projects.submit", $project->id) }}', {
            ...
            body: JSON.stringify({
                submission_note: formData.get('submission_note')
            })
        });
